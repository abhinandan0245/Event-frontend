...
import { motion, AnimatePresence } from "framer-motion";
import { Link } from "react-router-dom";
import { ChevronRight, Globe, MapPin, ArrowRight, Users } from "lucide-react";
    ...
    const fetchDestinations = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          const countries = Object.keys(formattedData);
        ...
        console.error("Failed to load destination menu data", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    destinationsArray.forEach((dest) => {
      ...
      if (!groupedData[country].statesMap[state]) {
        groupedData[country].statesMap[state] = {
          name: state,
          description: "Explore this beautiful region",
          icon: MapPin,
          cities: [],
        };
      }
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-2.5 truncate">
                      <CountryIcon
                        className={`w-4 h-4 shrink-0 ${isActive ? "text-pink-400" : "text-gray-400"}`}
                        strokeWidth={1.5}
                      />
                      <span className="truncate">{country}</span>
                    </span>
                    <ChevronRight
                      className={`w-3.5 h-3.5 shrink-0 transition-opacity ${isActive ? "opacity-100 text-pink-400" : "opacity-30"}`}
                    />
                  </button>
                );
              })}
                ...
                <div className="space-y-0.5">
                  {menuData[activeCountry].states.map((state) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${isActive ? "bg-amber-100" : "bg-amber-50"}`}
                        >
                          <StateIcon
                            className="w-4.5 h-4.5 text-amber-700"
                            strokeWidth={1.5}
                        ...
                        <ChevronRight
                          className={`w-3.5 h-3.5 shrink-0 ${isActive ? "text-amber-600" : "text-gray-300"}`}
                        />
                      </div>
                    );
                  })}
